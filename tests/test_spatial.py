...
def test_graph_from_config():
    ...
    assert g.position_of("dock_in") is not None
    assert g.position_of("cnc_1") is not None


def test_shortest_path_direct():
    config = load_config("configs/assembly_line_3station.yaml")
    g = SpatialGraph.from_config(config.facility)
    path = g.shortest_path("dock_in", "buf_1")
    assert path == ["dock_in", "buf_1"]


def test_shortest_path_multi_hop():
    config = load_config("configs/assembly_line_3station.yaml")
    g = SpatialGraph.from_config(config.facility)
    path = g.shortest_path("dock_in", "cnc_1")
    assert path == ["dock_in", "buf_1", "cnc_1"]


def test_shortest_path_to_exit():
    config = load_config("configs/assembly_line_3station.yaml")
    g = SpatialGraph.from_config(config.facility)
    path = g.shortest_path("qc_1", "dock_out")
    assert path == ["qc_1", "dock_out"]


def test_compute_route_positions():
    config = load_config("configs/assembly_line_3station.yaml")
    g = SpatialGraph.from_config(config.facility)
    route = g.compute_route("dock_in", "buf_1")
    assert len(route) == 2
