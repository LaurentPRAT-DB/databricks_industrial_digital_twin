...
export default function ScenarioReport({ scenarioId, scenarioName, onClose }: Props) {
  ...
  const runReport = useCallback(async () => {
    setLoading(true);
    setProgress('Running baseline + all what-ifs...');
    try {
      const res = await fetch(`/api/scenarios/${scenarioId}/run-report`, { method: 'POST' });
  ...
  return (
    <div className="w-[480px] bg-slate-800 border-l border-slate-700 flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-sm font-bold text-white uppercase tracking-wide">Results Report</h2>
        ...
        {!report && !loading && (
          <div className="text-center py-8">
            <p className="text-sm text-slate-400 mb-4">
              Run baseline + all saved what-if scenarios and compare results side-by-side.
            </p>
            <button
              onClick={runReport}
                      ...
                      {columns.map(c => (
                        <td key={c.key} className="py-2 px-2 text-right">
                          <div className="font-mono text-slate-200">
                            {wi.metrics[c.key]}{c.unit}
                          </div>
                          <DeltaBadge
                            baseline={report.baseline.metrics[c.key]}
                            value={wi.metrics[c.key]}
                  ...
                  {report.whatifs.length === 0 && (
                    <tr>
                      <td colSpan={columns.length + 1} className="py-4 text-center text-slate-500 text-xs">
                        No saved what-if scenarios. Use the What-If editor to create and save some.
                      </td>
                    </tr>
                  )}
                </tbody>
